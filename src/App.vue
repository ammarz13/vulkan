<template>
  <div id="app" >
    ...
    <!-- Info -->
    <div loading-container class="content">      
        ...
        try {
          ...
          if (result.status) {             
            this.$swal({
              icon: 'success',
              title: msg.title,
              text: msg.success ? msg.success : 'Blockchain confirmation successful!',
            ...
            this.$swal({
              icon: 'error',
              title: msg.title,
              text: msg.nosuccess ? msg.nosuccess : 'Blockchain confirmation error!',
              //footer: `<a href="${this.web3modal.explorerURL + 'tx/' + result.transactionHash}" target="_blank" class="text-highlight">${this.$options.filters.txHashShort(result.transactionHash)}</a>`,
          ...
          this.$swal({
            icon: 'error',
            title: msg.title,
            text: 'Blockchain confirmation error: ' + (error.data?.message ? error.message + ' ... ' + error.data?.message : error.message),
            confirmButtonColor: this.confirmButtonColor
          });
